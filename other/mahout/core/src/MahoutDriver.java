...
public final class MahoutDriver {
  ...
  public static void main(String[] args) throws Throwable {
    ...
    String[] hosts = argMap.get("-DrabbitHost");
    if (hosts.length == 1) {
        host = hosts[0].trim();
    }
    String[] exchanges = argMap.get("-DrabbitExchange");
    if (exchanges.length == 1) {
        exchange = exchanges[0].trim();
    }
    String[] dronJobInstanceIds = argMap.get("-DdronJobInstanceId");
    if (dronJobInstanceIds.length == 1) {
        dronJobInstanceId = dronJobInstanceIds[0].trim();
    }
